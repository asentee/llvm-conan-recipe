...
# Dictionary to map LLVM projects to their CMake module file which are
# capitalized differently. e.g. ClangConfig.cmake, LLDConfig.cmake
# Also tells whether the project has a module directory or not.
LLVM_PROJECTS = {
    "bolt":               {"has_mod_dir":True,  "mod_file":""},
    "clang":              {"has_mod_dir":True,  "mod_file":"ClangConfig.cmake"},
    "clang-tools-extra":  {"has_mod_dir":False, "mod_file":""}, 
    "compiler-rt":        {"has_mod_dir":True,  "mod_file":""},
    "cross-project-tests":{"has_mod_dir":False, "mod_file":""},
    "libc":               {"has_mod_dir":True,  "mod_file":"LibcConfig.cmake"},
    "libclc":             {"has_mod_dir":True,  "mod_file":""},
    "lld":                {"has_mod_dir":True,  "mod_file":"LLDConfig.cmake"},
    "lldb":               {"has_mod_dir":True,  "mod_file":"LLDBConfig.cmake"},
    "mlir":               {"has_mod_dir":True,  "mod_file":"MLIRConfig.cmake"},
    "openmp":             {"has_mod_dir":True,  "mod_file":""},
    "polly":              {"has_mod_dir":True,  "mod_file":"PollyConfig.cmake"}
}

def get_components_from_dotfile(project_name, dotfile):
    def node_labels(dot):
        label_replacements = {
    ...
    def node_starts_with_name(node):
        return node.startswith(project_name.lower()) or node.startswith(project_name.upper()) or node.startswith(project_name.capitalize())
    ...
    def node_dependencies(dot):
        ...
        for row in dot:
            ...
            if match_dep:
                ...
                if node_starts_with_name(node_label) and PurePosixPath(dependency).parts[-1] not in ignore_deps:
                    yield node_label, labels[match_dep.group(2)]
        # some components don't have dependencies
        for label in labels.values():
            if node_starts_with_name(label):
                yield label, None
...
class LLVMConan(ConanFile):
    ...
    url = "https://github.com/asentee/llvm-conan-recipe"
    ...
    # Specifies the minimum C++ standard
    @property
    def _min_cppstd(self):
        return 17

    # Specifies the minimum C++ compiler version
    @property
    ...
    # Path to generated graphviz file
    @property
    def _graphviz_file(self):
        return PurePosixPath(self.build_folder) / "llvm.dot"
    ...
    # Path to llvm src folder
    @property
    def _llvm_source_folder_path(self):
        return PurePosixPath(self.source_folder) / "llvm"

    # Path to cmake module directory of llvm 
    @property
    def _cmake_llvm_module_path(self):
        return PurePosixPath("lib") / "cmake" / "llvm"

    # Path to cmake module directory
    @property
    def _cmake_module_path(self):
        return PurePosixPath("lib") / "cmake"

    # Path to build info file for llvm
    @property
    def _build_info_file(self):
        return self._package_folder_path / self._cmake_llvm_module_path / "conan_llvm_build_info.json"

    # Path to build info file for an enabled project
    def _project_build_info_file(self, project):
        return self._package_folder_path / self._cmake_module_path / project / f"conan_{project}_build_info.json"

    @property
    def _build_module_file_rel_path(self):
        return self._cmake_llvm_module_path / f"conan-official-{self.name}-variables.cmake"

    # 
    # Build Info
    # 
    
    # Returns build info for llvm
    def _llvm_build_info(self):
        cmake_config = Path(self._package_folder_path / "lib" / "cmake" / "llvm" / "LLVMConfig.cmake").read_text("utf-8")
        components = get_components_from_dotfile("llvm", load(self, self._graphviz_file))
    ...
    # Returns build info for an enabled project
    def _llvm_project_build_info(self, project):
        components = get_components_from_dotfile(project, load(self, self._graphviz_file))
        return {"components": components}

    # Create the custom conan cmake build module that is supposed to provide the
    # user with information that gets removed by deleting the library generated
    # config file.
    def _create_cmake_build_module(self, build_info, project_build_info, module_file):
        # TODO: add info on individual projects that could be enabled

        def enabled_project_availability_flags():
            flags = ""
            for proj in self._enabled_projects:
                flags += f"set(LLVM_PROJECT_{proj.upper()}_ENABLED TRUE)"
            return flags
        ...
        content = textwrap.dedent(f"""\
            ...
            {enabled_project_availability_flags()}
    ...
    # Writes build info for an enabled project
    def _write_project_build_info(self, project):
        build_info = self._llvm_project_build_info(project)
        with open(self._project_build_info_file(project), "w", encoding="utf-8") as fp:
            json.dump(build_info, fp, indent=2)
        return build_info

    # Reads build info for llvm
    def _read_build_info(self) -> dict:
        with open(self._build_info_file, encoding="utf-8") as fp:
            return json.load(fp)

    # Reads build info for an enabled project
    def _read_project_build_info(self, project) -> dict:
        with open(self._project_build_info_file(project), encoding="utf-8") as fp:
            return json.load(fp)
    ...
    def validate(self):
        ...
        # TODO: Check whether the target option and enable_project option are valid
    ...
    def package(self):
        ...
        for proj in self._enabled_projects:
            if LLVM_PROJECTS[proj]["mod_file"] != "":
                rm(self, LLVM_PROJECTS[proj]["mod_file"], self._package_folder_path / "lib" / "cmake" / proj)
            project_build_info[proj] = self._write_project_build_info(proj)
    ...
    def package_info(self):
        ...
        if not self.options.shared:
            ...
            for proj in self._enabled_projects:
                project_build_info = self._read_project_build_info(proj)
                project_components = project_build_info["components"]

                for component_name, data in project_components.items():
                    self.cpp_info.components[component_name].set_property("cmake_target_name", component_name)
                    self.cpp_info.components[component_name].libs = [component_name]
                    self.cpp_info.components[component_name].requires = data["requires"]
                    self.cpp_info.components[component_name].system_libs = data["system_libs"]
